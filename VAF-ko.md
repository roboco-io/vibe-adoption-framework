...
VAF의 도입 여정은 준비 → 진단 → 파일럿 → 전환의 4단계로 구성된다. 이 여정은 한 번에 끝나는 프로그램이 아니라 반복되는 사이클이다. 하나의 파일럿 범위에서 사이클을 완주한 뒤, 다음 대상으로 다시 진입한다. 조직은 사이클을 반복할수록 역량 항목(5장)이 성숙해지고 전환 영역(4장)의 ToBe에 가까워진다.

```mermaid
flowchart LR
    P["1 준비<br/>(Prepare)"] --> A["2 진단<br/>(Assess)"]
    A --> PI["3 파일럿<br/>(Pilot)"]
    PI --> T["4 전환<br/>(Transform)"]
    T -->|"다음 대상으로<br/>사이클 반복"| P
```

여정에 들어가기 전에 한 가지 순서를 강조한다. **VAF는 진단보다 환경 준비를 먼저 둔다.** 전통적 컨설팅은 진단으로 시작하지만, VAF에서는 진단·분석·문서화의 수행 주체가 에이전트다. 에이전트가 일할 수 있는 환경을 먼저 갖추면, 이후의 모든 단계가 에이전트의 속도로 진행되고, 조직은 첫 단계에서부터 "에이전트와 함께 일하는" 경험을 시작하게 된다.

### 6.1 1단계: 준비 (Prepare)

**목표**: 에이전트가 조직의 환경에서 실제로 일할 수 있는 조건을 만든다.

**핵심 활동**:
- 파일럿 후보 시스템을 중심으로 접근 통로 구축 — 리포지토리, 이슈 트래커, 파일 저장소, 인프라(5.4 접근 통로)
- 도구 연결과 전용 자격증명 발급, 최소 권한 설계(5.4 도구 연결, 권한 관리)
- 에이전트가 테스트·빌드를 실행할 수 있는 실행 환경 확보(5.4 실행 환경)
- 보안 조직과의 초기 협의 — 권한·감사 체계를 설계에 반영(5.3 보안·컴플라이언스)

**산출물**: 에이전트 접근 가능 시스템 목록, 자격증명·권한 체계, 동작하는 실행 환경

**완료 기준**: 파일럿 대상 시스템에 대한 접근 통로와 실행 환경이 확보되고, 에이전트가 대표 업무 1건(예: 이슈 확인 → 코드 수정 → 테스트 → 리뷰 요청)을 사람의 중계 없이 끝까지 수행할 수 있다.

### 6.2 2단계: 진단 (Assess)

**목표**: 조직의 현안과 자산을 에이전트와 함께 문서화하고, AsIs와 ToBe의 갭을 정리한다.

**핵심 활동**:
- 경영진·현업·개발 조직 심층인터뷰로 비즈니스·IT 현안 문서화(5.5 현안 문서화) — 부록의 인터뷰 질문지 활용
- 기존 리포지토리의 에이전트 분석·문서화(5.5 리포지토리 분석·문서화)
- 리소스 현황 조사와 지식기반(LLM Wiki 등) 구축(5.5 지식기반 구축)
- 이상적 SDLC를 ToBe로 설정하고 AsIs와의 갭을 역량 항목의 언어로 정리(5.1 SDLC 갭 분석)
- 성과 기준선 측정 — 리드타임, 배포 빈도, 결함률, 리뷰 소요 시간(5.1 가치·ROI 측정)
- 파일럿 대상 확정(5.1 우선순위 포트폴리오)

**산출물**: 현안 문서, 리포지토리 분석 문서, 지식기반 v1, AsIs/ToBe 갭 목록, 성과 기준선, 파일럿 선정안

**완료 기준**: 현안 문서·리포지토리 문서·갭 목록·지식기반 v1이 완성되어, 파일럿 팀과 에이전트가 참조할 수 있는 상태다.

### 6.3 3단계: 파일럿 (Pilot)

**목표**: 선정된 범위에서 바이브 코딩 전환을 실제로 수행하고, 확산 가능한 형태의 학습을 얻는다.

**핵심 활동**:
- 파일럿 리포지토리에 규칙 파일·스킬·훅 구성(5.6 규칙 파일 관리, 스킬·훅 구성)
- 취향/비취향 정책 확정과 저장소 강제(5.3) — 파일럿에서 정책의 실효성을 검증
- 자동 리뷰·검증 파이프라인 가동(5.6)
- 파일럿 팀의 개발 방식 전환 — 신규 코드는 에이전트가 작성, 사람은 지시·리뷰·승인
- 역할 전환의 첫 적용과 역량 개발 실습(5.2)
- 주기적 회고 — 하네스 공백, 인지부채 상태, 지표 변화를 점검하고 하네스 개선 루프 가동(5.6)

**산출물**: 동작하는 하네스(규칙·스킬·훅·리뷰·파이프라인), 파일럿 성과 데이터, 확산용 플레이북(파일럿에서 검증된 절차의 일반화)

**완료 기준**: 파일럿 팀의 신규 코드가 에이전트 작성으로 전환되고, 하네스가 가동 중이며, 기준선 대비 성과 지표가 측정되고 있다.

### 6.4 4단계: 전환 (Transform)

**목표**: 파일럿의 검증된 방식을 전사로 확산하고, 새로운 개발 방식을 조직의 기본값으로 정착시킨다.

**핵심 활동**:
- 플레이북 기반의 팀별 순차 확산 — 각 팀마다 준비·진단의 축소판(접근 통로, 리포지토리 문서화, 규칙 구성)을 반복
- 오너십 체계와 인지부채 관리의 전사 운영(5.2) — 오너 지정, 이해 수준 결정, 정기 점검
- 역할·평가 체계 개편의 제도화(5.2 역할 전환)와 문화 정착 활동(5.2 문화 전환)
- 성과 지표의 상시 관측 — 리드타임, 배포 빈도, 변경 실패율·결함률, 리뷰 소요 시간
- 비용 관리 체계 운영(5.3) — 단위 성과당 비용 관점의 정기 리뷰
- 다음 사이클 기획 — 남은 갭과 새 현안으로 다음 준비·진단 단계 진입

**산출물**: 전사 확산 현황, 상시 성과 대시보드, 갱신된 갭 목록과 다음 사이클 계획

**완료 기준**: 대상 조직 전체로 확산이 완료되고, 사람이 코드를 손으로 쓰지 않는 상태가 기본값이 되었으며, 오너십·인지부채 관리 체계가 운영되고 있다.

### 6.5 여정 운영의 원칙

- **작게 완주하라**: 첫 사이클은 범위를 좁혀 끝까지 완주하는 것이 넓게 걸쳐 놓고 멈추는 것보다 낫다. 완주 경험이 확산의 연료다.
- **단계를 건너뛰지 마라**: 준비 없는 진단은 사람의 속도로 진행되고, 진단 없는 파일럿은 맥락 없는 코드를 낳고, 파일럿 없는 전환은 검증되지 않은 방식을 전사에 강요한다.
- **모든 단계에서 심층인터뷰로 확인하라**: 각 단계의 산출물은 이해관계자 인터뷰로 검증하며 컨텍스트를 보충한다. 문서가 현실과 어긋난 채 다음 단계로 넘어가는 것이 가장 비싼 실패다.
- **지표는 사이클마다 갱신하라**: 기준선 대비 변화를 사이클 단위로 보고하라. 경영진의 신뢰는 지표의 연속성에서 나온다.

## 7. 부록: 실행 키트

본문의 프레임워크를 실제 도입 활동으로 옮기기 위한 도구 모음이다. 진단 단계(6.2)의 어세스먼트와 인터뷰, 진단·파일럿 단계의 워크숍에서 사용한다.

### 7.1 Capability 성숙도 어세스먼트 체크리스트

각 역량 항목을 다음 3단계로 평가한다.

- **부재 (0)**: 해당 역량이 존재하지 않거나 개인의 임의 활동에 머문다
- **부분 (1)**: 일부 팀·일부 영역에서 작동하나 조직 표준이 아니다
- **정착 (2)**: 조직 표준으로 문서화·강제되며 지속적으로 운영된다

판별 질문에 "일부만 그렇다"면 부분, "조직 전체가 그렇다"면 정착이다. 어세스먼트 결과는 갭 목록(5.1 SDLC 갭 분석)의 입력이 된다.

**Strategy**

| Capability | 판별 질문 |
|---|---|
| 도입 전략 | 도입의 목적과 우선순위가 문서로 존재하고 경영진 스폰서가 지정되어 있는가? |
| 가치·ROI 측정 | 리드타임·결함률 등 개발 성과의 기준선이 측정되어 있는가? |
| SDLC 갭 분석 | ToBe SDLC와 AsIs의 갭이 문서로 정리되어 있는가? |
| 우선순위 포트폴리오 | 전환 대상과 순서가 근거와 함께 결정되어 있는가? |

**People & Ownership**

| Capability | 판별 질문 |
|---|---|
| 오너십 체계 | 모든 리포지토리·시스템에 지정된 오너가 있고 목록이 저장소에 있는가? |
| 인지부채 관리 | 시스템별 요구 이해 수준이 조직의 결정으로 문서화되어 있는가? |
| 역할 전환 | 직무·평가 기준에 코드 작성량 외의 기준(지휘·오너십)이 반영되어 있는가? |
| 역량 개발 | 에이전트 활용 교육이 실습 기반 커리큘럼으로 존재하는가? |
| 문화 전환 | 에이전트와 일하는 방식이 기본값이며 실패 사례가 공유되는가? |

**Governance & Guardrails**

| Capability | 판별 질문 |
|---|---|
| 취향/비취향 구분 | 무엇이 강제 대상인지 목록으로 결정되어 있고 이견 판정 기구가 있는가? |
| 정책의 저장소 강제 | 비취향 정책이 규칙 파일·훅·CI로 구현되어 우회 불가능한가? |
| 보안·컴플라이언스 | 에이전트 권한·감사 추적 체계가 보안 조직과 합의되어 있는가? |
| 비용 관리 | 에이전트 비용이 팀 단위로 측정되고 성과 지표와 함께 검토되는가? |

**Agent Environment**

| Capability | 판별 질문 |
|---|---|
| 접근 통로 | 에이전트가 리포지토리·이슈 트래커·인프라에 접근해 업무 흐름을 완주할 수 있는가? |
| 도구 연결 | 사내 시스템이 표준 프로토콜(MCP 등)의 도구로 연결되어 있는가? |
| 실행 환경 | 에이전트가 테스트·빌드를 스스로 실행하고 결과를 피드백받는가? |
| 권한 관리 | 에이전트 전용 자격증명과 최소 권한 체계가 운영되는가? |

**Knowledge**

| Capability | 판별 질문 |
|---|---|
| 지식기반 구축 | 에이전트가 탐색 가능한 지식기반이 존재하고 갱신 절차가 있는가? |
| 리포지토리 분석·문서화 | 전환 대상 리포지토리의 구조·흐름·문제가 문서화되어 있는가? |
| 현안 문서화 | 비즈니스·IT 현안이 인터뷰 기반으로 문서화되어 있는가? |
| 컨텍스트 상시 활용 체계 | 지식이 규칙 파일·스킬을 통해 에이전트에게 자동 공급되는가? |

**Harness Engineering**

| Capability | 판별 질문 |
|---|---|
| 규칙 파일 관리 | 리포지토리별 규칙 파일이 존재하고 조직 공통 규칙과 계층화되어 있는가? |
| 스킬·훅 구성 | 반복 절차가 스킬로, 강제 규칙이 훅으로 구현되어 버전 관리되는가? |
| 자동 리뷰 | AI 1차 리뷰가 가동되고 지적의 처리 절차가 있는가? |
| 검증 파이프라인 | 병합 전 자동 검증이 있고 에이전트가 사전에 스스로 돌릴 수 있는가? |
| 하네스 개선 루프 | 결함·장애가 규칙·훅·테스트의 변경으로 환류되는가? |

### 7.2 심층인터뷰 질문지

진단 단계(6.2)에서 사용한다. 인터뷰의 설계·기록·정리는 에이전트가 보조하며, 영역별로 대상자를 달리한다(비즈니스 현안: 경영진·사업 책임자 / IT 현안·SDLC: 개발 리더·팀 / 지식 관리: 전 대상).

**비즈니스 현안**

1. 지금 사업에서 가장 풀고 싶은 문제 세 가지는 무엇인가? 그중 소프트웨어가 병목인 것은?
2. 개발 조직에 대한 가장 큰 불만 또는 아쉬움은 무엇인가? (속도, 품질, 소통, 비용)
3. 6개월 안에 달라진다면 사업적으로 가장 가치 있는 변화는 무엇인가?
4. 과거에 시도했다가 실패한 개발 개선 활동이 있는가? 왜 실패했다고 보는가?

**IT 현안**

1. 현재 개발 조직의 가장 큰 병목은 어디인가? (기획, 개발, 리뷰, QA, 배포, 운영)
2. 손대기 두려운 시스템은 무엇이며, 왜 두려운가?
3. 특정 개인이 떠나면 멈추는 시스템·업무가 있는가?
4. 반복적으로 발생하는 장애·결함의 패턴이 있는가?

**SDLC 현황 (AsIs)**

1. 아이디어가 배포까지 도달하는 실제 경로와 소요 시간은? (최근 사례로)
2. 코드 리뷰는 누가, 어떤 기준으로, 얼마나 걸려서 하는가?
3. 테스트·배포는 얼마나 자동화되어 있는가? 수동 단계는 무엇인가?
4. 코딩 규약·보안 정책은 어디에 있고, 실제로 지켜지는가? 무엇이 강제되고 무엇이 권고인가?
5. 현재 AI 도구를 쓰는 구성원은 얼마나 되고, 어떻게 쓰고 있는가?

**지식 관리 현황**

1. 새 팀원이 합류하면 무엇으로 시스템을 배우는가? 얼마나 걸리는가?
2. 시스템 문서는 어디에 있고, 마지막으로 갱신된 것은 언제인가?
3. "이건 ○○님만 안다"에 해당하는 지식은 무엇인가?
4. 의사결정(왜 이렇게 만들었나)의 기록이 남아 있는가?

### 7.3 워크숍 가이드

**진단 워크숍** — 진단 단계 중반, 인터뷰·분석 결과가 모인 시점에 실시한다.

- **목적**: 어세스먼트 결과와 현안 문서를 이해관계자와 함께 검증하고, ToBe SDLC와 갭의 우선순위를 합의한다
- **참석자**: 경영진 스폰서, 개발 리더, 보안·거버넌스 책임자, 파일럿 후보 팀 리드
- **아젠다** (반나절): ① 어세스먼트 결과 공유(관점별 성숙도) ② 현안 문서 검증·보완 ③ ToBe SDLC 초안 논의 ④ 갭 우선순위와 파일럿 대상 합의
- **산출물**: 합의된 갭 목록과 우선순위, 파일럿 선정안, ToBe SDLC 합의안

**파일럿 착수 워크숍** — 파일럿 단계 시작 시 파일럿 팀과 실시한다.

- **목적**: 파일럿 팀이 새로운 개발 방식·하네스·역할을 이해하고, 성공 기준에 합의한다
- **참석자**: 파일럿 팀 전원, 하네스 구성 담당, 경영진 스폰서(시작·마무리 참석)
- **아젠다** (1일): ① 관통 원리와 오너십 원칙 공유 ② 파일럿 리포지토리의 규칙·하네스 시연 ③ 실습 — 실제 백로그 아이템을 에이전트와 완주 ④ 역할·회고 주기·성공 기준(완료 기준과 지표) 합의
- **산출물**: 파일럿 운영 합의서(범위, 역할, 회고 주기, 성공 기준)

### 7.4 용어집

| 용어 | 정의 |
|---|---|
| 바이브 코딩 (Vibe Coding) | 사람이 의도를 제시하고 AI 에이전트가 코드를 작성하는 개발 방식 |
| 에이전트 (Agent) | 도구를 사용해 다단계 작업을 자율 수행하는 AI 시스템. 본 문서에서는 주로 코딩 에이전트 |
| 관점 (Perspective) | 도입에 필요한 조직 역량의 분류 축. VAF는 6개 관점을 정의한다 |
| 역량 항목 (Capability) | 관점을 구성하는 진단·계획의 단위 역량 |
| 전환 영역 (Transformation Domain) | 도입으로 실제 변화가 일어나는 영역. 코드 생산, 품질 보증, 지식, 조직과 역할 |
| 도입 여정 (Adoption Journey) | 준비→진단→파일럿→전환의 4단계 반복 사이클 |
| 하네스 (Harness) | 사람이 하던 리뷰·가드레일을 AI가 대신 수행하도록 만든 체계. 규칙 파일·스킬·훅·자동 리뷰·검증 파이프라인의 총칭 |
| 하네스 엔지니어링 (Harness Engineering) | 하네스를 구축·운영·개선하는 공학 |
| 인지부채 (Cognitive Debt) | 시스템에 대한 사람의 이해와 실제 시스템 사이의 간극. 관리 대상이며 제거 대상이 아니다 |
| 기술부채 (Technical Debt) | 나중에 갚기로 하고 미룬 설계·구현 품질 |
| 취향/비취향 (Preference / Non-preference) | 개인 재량 허용 영역과 조직 강제 영역의 구분. 비취향은 저장소와 함께 강제된다 |
| 오너십 원칙 (Ownership Principle) | 코드는 에이전트가 쓰고 책임은 사람이 지며, 오너는 언제든 답할 수 있어야 한다는 원칙 |
| 규칙 파일 (Rules File) | 리포지토리에 두는 에이전트용 규칙 문서 (예: AGENTS.md, CLAUDE.md) |
| 스킬 (Skill) | 에이전트가 조직의 방식대로 반복 작업을 수행하도록 패키징한 절차 |
| 훅 (Hook) | 에이전트의 특정 동작 시점에 강제로 실행되는 검사·차단 장치 |
| LLM Wiki | 에이전트가 탐색·활용하기 좋은 형태로 상호 참조되게 구축한 문서형 지식기반 |
| MCP (Model Context Protocol) | 에이전트와 외부 도구·데이터를 연결하는 표준 프로토콜 |
| AsIs / ToBe | 현재 상태 / 목표 상태 |
| 기준선 (Baseline) | 도입 전에 측정해 두는 성과 지표의 출발점 |
| DORA 지표 | 리드타임, 배포 빈도, 변경 실패율, 복구 시간 등 소프트웨어 딜리버리 성과 지표 |
